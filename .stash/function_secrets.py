    ...
    def __init__(self):
        try:
                ...
                credentials = {
                    "usernames": {
                        ...
                        for username, email, name, password in zip(
                            ...
                            secrets["credentials"]["emails"],
                            secrets["credentials"]["names"],
            ...
            self.authenticator = stauth.Authenticate(
                credentials,
                st.secrets["cookie"]["name"] if hasattr(st, 'secrets') and st.secrets is not None else secrets["cookie"]["name"],
                st.secrets["cookie"]["key"] if hasattr(st, 'secrets') and st.secrets is not None else secrets["cookie"]["key"],
                st.secrets["cookie"]["expiry_days"] if hasattr(st, 'secrets') and st.secrets is not None else secrets["cookie"]["expiry_days"]
            )
        ...
        try:
            ...
            if hasattr(st, 'secrets') and st.secrets is not None:
                ...
                try:
                    ...
                    self.AMIVOICE_APP_KEY = st.secrets["AMIVOICE_APP_KEY"]
                except Exception as e:
                    st.error("Streamlit Cloudのシークレット設定を確認してください。")
                    st.error("OPENAI_API_KEYとAMIVOICE_APP_KEYが設定されているか確認してください。")
                    st.stop()
            else:
                # ローカル環境
                try:
                    ...
                    self.AMIVOICE_APP_KEY = secrets["AMIVOICE_APP_KEY"]
                except Exception as e:
                    st.error("ローカル環境の.streamlit/secrets.tomlファイルを確認してください。")
                    st.error("OPENAI_API_KEYとAMIVOICE_APP_KEYが設定されているか確認してください。")
                    st.stop()
                
        except Exception as e:
            st.error(f"APIキーの読み込みに失敗しました: {str(e)}")
            st.error("環境設定を確認してください。")
            st.stop()
