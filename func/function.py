...
class Auth:
    def __init__(self):
        try:
            ...
            username = str(secrets["credentials"]["usernames"][0])
            credentials = {
                "usernames": {
                    username: {
                        "email": str(secrets["credentials"]["emails"][0]),
                        "name": str(secrets["credentials"]["names"][0]),
                        "password": str(secrets["credentials"]["passwords"][0])
                    }
                }
            }
            cookie_name = str(secrets["cookie"]["name"])
            cookie_key = str(secrets["cookie"]["key"])
            cookie_expiry_days = int(secrets["cookie"]["expiry_days"])

            # # デバッグ情報
            # st.write("認証情報の構造:")
            # st.write(credentials)
            # st.write("Cookie設定:")
            # st.write(f"名前: {cookie_name}")
            # st.write(f"キー: {cookie_key}")
            # st.write(f"有効期限: {cookie_expiry_days}日")
...
class Functions:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.OPENAI_API_KEY = str(secrets["OPENAI_API_KEY"])
            self.AMIVOICE_APP_KEY = str(secrets["AMIVOICE_APP_KEY"])
            
            if not self.OPENAI_API_KEY or not self.AMIVOICE_APP_KEY:
                st.error("APIキーが設定されていません。")
                st.error("secrets.tomlでOPENAI_API_KEYとAMIVOICE_APP_KEYを設定してください。")
                st.stop()
